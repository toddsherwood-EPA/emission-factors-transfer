"""
This script reads in, from the parameters input file, the SAFE NPRM tailpipe emission factors. It also reads in the tailpipe emission factors
from the OMEGA_ICBT_Inputs file used in the 2016 Proposed Determination analysis done by EPA. The script then replaces the SAFE NPRM emission
factors for model years 2010 and later with those from the proposed determination analysis. It then sets to zero, technically to NAN, the 2010
and later emission factors for the LDT2b/3 entries since EPA did not use those in the proposed determination. If NHTSA wants to continue use of
the SAFE NPRM values for 2010 and later model year LDT2b/3, they can simply leave those columns as they were in the SAFE NPRM. The 2010 and later
model year data are then appended back to the SAFE NPRM values for model years 2009 and earlier - EPA did not use emission factors for 2009 and
earlier so they cannot be inserted into the CAFE model format via this script. The output is two csv files, one for gasoline emission factors and
one for diesel.

The script can be found on GitHub at https://github.com/toddsherwood-EPA/emission-factors-transfer
"""
...
safe_nprm_set_to_nan = ['LDT2b/3_CO', 'LDT2b/3_VOC', 'LDT2b/3_NOx', 'LDT2b/3_SO2', 'LDT2b/3_PM2.5',
...
def main():
    ...
    # Most of this section is not necessary for the purpose of this script. But, if re-used someday in another way and merging on MY & Age the
    ...
    for metric in safe_nprm_set_to_nan:
